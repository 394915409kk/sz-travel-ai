...
apps/backend/init_db.py              本地开发/测试数据库初始化辅助
...
scripts/migrate_db.py                安全迁移入口：检查、upgrade、stamp-existing
scripts/backup_sqlite.py             SQLite 备份脚本
scripts/restore_sqlite.py            SQLite 恢复脚本
tests/                               自动化测试
```
...
如需本地示例产品数据，可在 development 环境额外执行 `python -m apps.backend.init_db`。
...
| `SQLITE_BACKUP_DIR` | `backups/` | SQLite 备份目录 |
| `DATABASE_URL` | 空 | 预留给未来 PostgreSQL 等数据库连接；不要写入真实生产连接 |
| `AUTO_INIT_DB_ON_STARTUP` | `true`（非生产） | 非生产环境是否允许启动时辅助初始化；production 永远不会自动初始化 |
